...
class PokerGame:
    ...
    def _is_round_complete(self) -> bool:
        ...
        active_players = [p for p in non_folded if not p.is_all_in]
        if len(non_folded) <= 1:
            return True
    ...
    def _get_best_hand_cards(self, cards: List[Card]) -> List[Card]:
        """Return the actual five cards that make up the best hand."""
        from itertools import combinations
    ...
    def _award_pot(self, winners: List[Player]):
        ...
        if not side_pots and self.pot > 0:
            side_pots.append({'size': self.pot, 'eligible': winners, 'bet_level': 0})

        total_winnings = {p.id: 0 for p in active_players}
        for side_pot in side_pots:
            eligible = side_pot['eligible']
            if not eligible:
                continue
            if len(eligible) == 1:
                ...
                hand_evaluations = [
                    (p, self._get_best_hand(p.hand + self.community_cards))
                    for p in eligible
                ]
                hand_evaluations.sort(key=lambda x: x[1], reverse=True)
                best_hand = hand_evaluations[0][1]
            ...
            split_amount = side_pot['size'] // len(pot_winners)
            remainder = side_pot['size'] % len(pot_winners)
            for i, winner in enumerate(pot_winners):
                amount = split_amount + (1 if i < remainder else 0)
                winner.chips += amount
                total_winnings[winner.id] += amount

        self.side_pots = [
            {
                'amount': pot['size'],
                'eligible_players': [p.id for p in pot['eligible']]
            }
        ...
        self.winners = [{
            ...
            'name': player.name,
            'amount': amount,
            'hand': [c.to_dict() for c in self._get_best_hand_cards(player.hand + self.community_cards)]
        } for player in active_players if (amount := total_winnings.get(player.id, 0)) > 0]
